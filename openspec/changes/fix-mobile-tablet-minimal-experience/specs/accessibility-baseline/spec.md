...
On viewports below the desktop gate, every interactive element MUST have at least a 44px by 44px hit area.

#### Scenario: User taps header buttons on mobile
- **WHEN** a user views the header on a mobile viewport
- **THEN** the hamburger, the toggle, and the "Pedir asesoría" CTA each have at least 44px hit area
...
Every form input on the page MUST have a visible label or an `aria-label` attribute. Required fields MUST be marked with `aria-required="true"`.
