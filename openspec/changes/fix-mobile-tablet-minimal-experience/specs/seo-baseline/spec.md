...
The page SHALL embed a `LocalBusiness` JSON-LD script with name, address, telephone, and `sameAs` (Instagram). Address fields MUST use the real `ADDRESS` constant. Telephone MUST use the real `PHONE` constant.

#### Scenario: Search engine reads structured data
- **WHEN** a search engine parses the page
- **THEN** the page exposes a `LocalBusiness` JSON-LD object with the real address and phone
