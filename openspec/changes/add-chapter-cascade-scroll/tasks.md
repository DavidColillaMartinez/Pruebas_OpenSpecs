...
- [x] 7.6 Rellenar `src/data/reviewsContent.js` con autor/estrellas/texto de las 7 reseñas verificadas cuando las aporte el propietario (enlaces ya registrados en el fichero). → Cubierto por 7.7: el propietario aportó finalmente el export completo con 13 reseñas.
- [x] 7.7 (2026-09-08) Revisión del propietario: el export de Google aportó 13 reseñas reales (todas 5★) con autor, fecha, texto e imágenes; se rellenan `reviewsContent.js` y `public/reviews/`. Sin URLs individuales, «Ver en Google» enlaza a la ficha de empresa para las 13. La reseña de Laura Hernandez Parrado no tiene texto y se muestra sin inventar copia (pendiente de aceptación visual en navegador).
- [x] 7.8 (2026-09-08) Ajustes de coreografía pedidos por el propietario: Inicio con separación fluida intermedia `mt-[clamp(3.5rem,14vh,10rem)]`; Quiénes somos pasa a 3 pasos con Estudio/Método imagen+texto simultáneos (imagen izquierda) y Oficio texto izquierda + imagen derecha lateral; Colección pasa a 3 pasos: bloque izquierdo fluido, pausa larga única y columna derecha en cascada corta (pendiente de aceptación visual en navegador).
