...
#### Scenario: Volver a un capítulo con cascada repetible
- **WHEN** el usuario regresa a Quiénes somos, Colección o Visión
- **THEN** el capítulo vuelve a ejecutar su cascada completa desde el inicio, con la pausa inicial de un segundo sobre el titular

#### Scenario: Volver a un capítulo ya revelado sin repetición
- **WHEN** el usuario regresa a un capítulo completado que no está en la lista de repetición (Inicio, Opiniones, Contacto)
...
- **THEN** los tres pasos permanecen ocultos, y al completarse la animación aparecen escalonados (0/320/640 ms) sin pausa adicional

#### Scenario: Posición del bloque de pasos
- **WHEN** los pasos de Inicio se revelan
- **THEN** el bloque fluye bajo el H1 con separación fija, y el espacio sobrante queda por debajo del grid, no entre el H1 y el grid
...
#### Scenario: Cascada coreografiada de Quiénes somos
- **WHEN** el usuario entra en Quiénes somos
- **THEN** en cinco pasos encadenados aparecen: texto 1 desde arriba, imagen 1 desde arriba, imagen 2 desde la izquierda junto con texto 2 desde la derecha, texto 3 desde abajo, e imagen 3 desde abajo con desfase
