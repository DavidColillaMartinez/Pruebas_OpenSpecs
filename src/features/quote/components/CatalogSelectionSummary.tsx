...
function SelectionLine({ line, compact = false }: { line: QuoteSelectionLine; compact?: boolean }) {
  ...
  return (
    <li className={compact ? 'border-b border-border-hairline/10 py-4 last:border-b-0' : 'border-b border-border-hairline/10 py-4 last:border-b-0'}>
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-3 flex items-center gap-2">
            ...
            <button type="button" onClick={() => removeLine(key)} className="text-xs font-semibold text-secondary underline-offset-4 hover:text-primary hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-clay">Eliminar</button>
          </div>
        </div>
      </div>
    </li>
...
function SummaryContent({ headingId, onClose, drawer = false }: { headingId: string; onClose?: () => void; drawer?: boolean }) {
  ...
  return (
    <div className={drawer ? 'flex h-full flex-col' : ''}>
      ...
      <Link to="/presupuesto" onClick={onClose} className="mt-6 inline-flex min-h-11 items-center justify-center rounded-full border border-border-hairline/25 px-4 text-sm font-semibold transition-colors hover:border-ink hover:bg-ink hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-clay">Ir a presupuesto{count > 0 ? ` (${count})` : ''}</Link>
    </div>
  );
}
  ...
  return (
    <>
      <aside className="hidden xl:block" aria-label="Resumen de mis selecciones">
        <div className="sticky top-6 max-h-[calc(100svh-3rem)] overflow-y-auto border-y border-border-hairline/10 py-5">
