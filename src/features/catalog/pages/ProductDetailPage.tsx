...
function RoyoConfiguration({ product }: { product: ProductDetail }) {
  ...
  return (
    <section className="mt-10 border-t border-border-hairline/10 pt-8" aria-labelledby="royo-configuration-heading">
      <h2 id="royo-configuration-heading" className="font-display text-3xl">Configuración del mueble</h2>
      {modularNotice && <p className="mt-4 max-w-3xl leading-relaxed text-secondary">{modularNotice}</p>}
      {(moduleTypes.length > 0 || presentationTypes.length > 0) && (
        <dl className="mt-5 grid gap-4 sm:grid-cols-2">
          {moduleTypes.length > 0 && <div><dt className="text-sm font-semibold text-secondary">Tipos de módulo disponibles</dt><dd className="mt-1">{moduleTypes.join(', ')}</dd></div>}
          {presentationTypes.length > 0 && <div><dt className="text-sm font-semibold text-secondary">Tipos de presentación</dt><dd className="mt-1">{presentationTypes.join(', ')}</dd></div>}
        </dl>
      )}
      {(depths || measureOptions) && (
        <dl className="mt-5 grid gap-4 sm:grid-cols-2">
          {depths && <div><dt className="text-sm font-semibold text-secondary">Profundidades informativas</dt><dd className="mt-1">{Object.entries(depths).map(([key, value]) => `${key}: ${String(value)} cm`).join(' · ')}</dd></div>}
          {measureOptions && <div><dt className="text-sm font-semibold text-secondary">Medidas informativas</dt><dd className="mt-1">{Object.entries(measureOptions).map(([key, value]) => `${key}: ${stringValues(value).join(', ')}`).join(' · ')}</dd></div>}
        </dl>
      )}
    </section>
  );
    ...
    <>
      <div className="grid gap-10 lg:grid-cols-[1.05fr_0.95fr]">
        ...
        <div>
          ...
          {product.supplierName && <p className="mt-3 text-secondary">Proveedor: {product.supplierName}</p>}
          {product.subcategory && <p className="mt-1 text-secondary">{product.subcategory}</p>}
          {product.description && <p className="mt-6 whitespace-pre-line leading-relaxed text-secondary">{product.description}</p>}
           <div className="mt-8">
            {isDuplach
           ...
           {selectedUnit && selectedSnapshot && <p className="mt-5 text-sm text-secondary" aria-live="polite">Selección: {variantLabel || selectedSnapshot.reference || 'Variante completa'} · Referencia {selectedSnapshot.reference || 'no publicada'}</p>}
           {!selectedUnit && isDuplach && <p className="mt-5 text-sm text-secondary" role="status">Completa la configuración seleccionada para poder añadir esta variante al presupuesto.</p>}
           {!selectedUnit && !isDuplach && <p className="mt-5 text-sm text-secondary" role="status">Esta ficha no tiene una variante pública completa seleccionable.</p>}
           <div className="mt-8 flex flex-wrap gap-3">
            <button type="button" disabled={!selectedUnit} onClick={addCurrentSelection} className="inline-flex min-h-12 items-center justify-center rounded-full border border-border-hairline/20 px-5 text-sm font-semibold transition-colors hover:border-border-hairline/60 disabled:cursor-not-allowed disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-clay">Añadir al presupuesto</button>
            <Link to="/presupuesto" className="inline-flex min-h-12 items-center justify-center rounded-full border border-border-hairline/20 px-5 text-sm font-semibold transition-colors hover:border-border-hairline/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-clay">Ver presupuesto</Link>
           </div>
...
<section className="mt-14 border-t border-border-hairline/10 pt-8" aria-labelledby="product-details-heading">
          <h2 id="product-details-heading" className="font-display text-3xl">
            ...
            >
              ...
              <span aria-hidden="true" className="text-lg font-normal leading-none text-secondary/70">{detailsOpen ? '−' : '+'}</span>
            </button>
          </h2>
          <div id="product-details-content" hidden={!detailsOpen}>
            <dl className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {productFacts.map(([key, value]) => <div key={String(key)}><dt className="text-sm font-semibold text-secondary">{key}</dt><dd className="mt-1">{typeof value === 'boolean' ? value ? 'Sí' : 'No' : String(value)}</dd></div>)}
              {selectionFacts.map(([key, value]) => <div key={key}><dt className="text-sm font-semibold text-secondary">{key === 'dimension' || key === 'measure' ? 'Medida' : key === 'finish' ? 'Acabado' : key === 'version' ? 'Versión' : key === 'has_led' ? 'LED' : key.replaceAll('_', ' ')}</dt><dd className="mt-1">{typeof value === 'boolean' ? value ? 'Sí' : 'No' : String(value)}</dd></div>)}
              {specs.map(([key, value]) => <div key={key}><dt className="text-sm font-semibold text-secondary">{key}</dt><dd className="mt-1">{readableDetailValue(value)}</dd></div>)}
              {product.availableFinishes.length > 0 && <div><dt className="text-sm font-semibold text-secondary">Acabados</dt><dd className="mt-1">{product.availableFinishes.join(', ')}</dd></div>}
              {product.availableMeasures.length > 0 && <div><dt className="text-sm font-semibold text-secondary">Medidas</dt><dd className="mt-1">{product.availableMeasures.join(', ')}</dd></div>}
            </dl>
          </div>
        </section>
      )}
      ...
      {product.commercialOffers.length > 0 && (
        <section className="mt-10 border-t border-border-hairline/10 pt-8" aria-labelledby="commercial-offers-heading">
          ...
          <ul className="mt-5 grid gap-3 sm:grid-cols-2">
            {product.commercialOffers.map((offer) => {
              ...
              return (
                <li key={offer.id} className="rounded-xl border border-border-hairline/10 bg-surface-elevated p-4">
                  <div className="flex items-start justify-between gap-4">
                    <p className="font-semibold">{offer.offerType || 'Conjunto'}</p>
                    <span className="rounded-full bg-elevated-hover px-2 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-secondary">Oferta</span>
                  </div>
                  <p className="mt-2 text-sm text-secondary">{offer.variants.length} acabados disponibles</p>
                  {references.length > 0 && (
                    <p className="mt-2 text-xs leading-relaxed text-secondary/80">Referencias: {references.slice(0, 3).join(', ')}{references.length > 3 ? '…' : ''}</p>
                  )}
...
export function ProductDetailPage() {
  ...
  useEffect(() => {
    ...
    if (state.status === 'not-found') {
      return applyRouteMeta({
        title: 'Producto no encontrado · AREA LRMQ',
        description: 'No hemos encontrado una ficha pública para este producto. Explora el catálogo completo de AREA LRMQ.',
        canonicalPath: null,
        noindex: true,
      });
    }
  ...
  return (
    <main className="min-h-screen bg-surface px-5 py-10 text-primary sm:px-8" id="product-content">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          {returnTo ? (
            <Link to={`/productos${returnTo}`} className="inline-flex min-h-11 items-center gap-2 self-start rounded-full border border-border-hairline/15 bg-surface-elevated px-4 text-sm font-semibold shadow-soft transition-all duration-200 ease-out hover:-translate-y-0.5 hover:border-border-hairline/40 hover:shadow-lift focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-clay focus-visible:ring-offset-2 focus-visible:ring-offset-surface">
              <span aria-hidden="true">←</span> Volver a resultados
            </Link>
          ) : (
            <Link to="/productos" className="inline-flex min-h-11 items-center gap-2 self-start rounded-full border border-border-hairline/15 bg-surface-elevated px-4 text-sm font-semibold shadow-soft transition-all duration-200 ease-out hover:-translate-y-0.5 hover:border-border-hairline/40 hover:shadow-lift focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-clay focus-visible:ring-offset-2 focus-visible:ring-offset-surface">
