...
function FilterGroups({ facets, filters, profile, onToggle, openGroups, expandedGroups, onToggleGroup, onToggleExpanded, idPrefix }: FilterGroupsProps) {
  ...
  if (groups.length === 0) return <p className="text-sm leading-relaxed text-secondary">Las opciones aparecerán cuando el catálogo las devuelva.</p>;
        ...
        return (
          <fieldset key={key}>
            <legend className="w-full">
              <button
                type="button"
                 className="flex min-h-12 w-full items-center gap-3 border-b border-border-hairline/10 px-1 text-left text-xs font-semibold uppercase tracking-[0.16em] text-secondary transition-colors duration-200 ease-out hover:border-border-hairline/35 hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-clay focus-visible:ring-offset-2"
                aria-expanded={open}
                aria-controls={contentId}
                onClick={() => onToggleGroup(key)}
              >
                 <span className="min-w-0 flex-1">{getCatalogFacetLabel(key, profile) || facetLabels[key]}</span>
                <span aria-hidden="true" className="text-lg font-normal leading-none text-secondary/70">{open ? '−' : '+'}</span>
              </button>
            </legend>
            <div id={contentId} hidden={!open} className="space-y-1 px-1 pb-2 pt-1">
              {visibleOptions.map((option, optionIndex) => {
                ...
                return (
                  <label key={option.value} className="group flex min-h-11 cursor-pointer items-center gap-3 rounded-md px-2 py-1 text-sm text-secondary transition-colors duration-200 ease-out hover:bg-elevated-hover hover:text-primary">
                    <input
                      type="checkbox"
                      checked={checked}
              ...
              {options.length > 8 && (
                <button
                  type="button"
                  className="px-2 pt-2 text-xs font-semibold text-secondary underline-offset-4 hover:text-primary hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-clay"
  ...
  return (
    <>
      <aside aria-label="Filtros del catálogo" className="hidden lg:block">
           <div className="sticky top-6 max-h-[calc(100svh-3rem)] overflow-y-auto overscroll-contain border-y border-border-hairline/10 py-5">
          <h2 className="font-display text-2xl">Filtrar</h2>
           <p className="mt-1 text-xs text-secondary">Abre una categoría para explorar sus opciones. Las cantidades siguen la consulta activa cuando hay datos suficientes.</p>
          <div className="mt-6">
