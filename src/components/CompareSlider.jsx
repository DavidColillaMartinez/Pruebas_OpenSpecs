export function CompareSlider({ videoRef, sliderRef, sliderX, onPointerDown, onKeyDown, isCompare, videoSrc, poster, finalImage, finalImageAlt, children }) {
  return (
    ...
    >
      <video ref={videoRef} src={videoSrc} muted playsInline preload="metadata" poster={poster} className="absolute inset-0 h-full w-full rounded-[1.2rem] bg-surface-strong object-cover" aria-label="Video de boceto dibujándose" />
      {isCompare && (
        <div className="absolute inset-0" style={{ clipPath: `inset(0 0 0 ${sliderX * 100}%)` }}>
          <img src={finalImage} alt={finalImageAlt} className="absolute inset-0 h-full w-full rounded-[1.2rem] bg-surface-strong object-contain" draggable={false} loading="lazy" />
        </div>
      )}
      {isCompare && (
        <div className="absolute bottom-0 top-0 w-0.5 bg-clay shadow-lg pointer-events-none" style={{ left: `${sliderX * 100}%` }}>
          <div className="absolute left-1/2 top-1/2 grid h-14 w-14 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border border-clay/30 bg-surface-strong text-action-foreground shadow-lift">
